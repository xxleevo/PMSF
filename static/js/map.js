...
var tileservers = '' // eslint-disable-line no-unused-vars
...
function pokemonLabel(item) {
    ...
    if (!noPokePVPStats && Store.get('showPokePVPStats') === true && pvpGreatLeague !== null && pvpUltraLeague !== null && pvpGreatLeague !== undefined && pvpUltraLeague !== undefined) {
        pvpGreatLeague = JSON.parse(item['pvp_gl'])
        pvpUltraLeague = JSON.parse(item['pvp_ul'])
        ...
        for (var j = 0; j < iterationLength; j++) {
            if (pvpUltraLeague[j]['rank'] !== null) {
                pvpMonStringUltra +=
                    '<div>Pokemon No.' + pvpUltraLeague[j]['pokemon'] + '-' + pvpUltraLeague[j]['form'] + ': ' + parseFloat(pvpUltraLeague[j]['percentage'] * 100).toFixed(1) + '%(#' + pvpUltraLeague[j]['rank'] + ') @ ' + pvpUltraLeague[j]['cp'] + i8ln('CP') + ', ' + i8ln('Level') + pvpUltraLeague[j]['level'] + '</div>'
            } else {
                pvpMonStringUltra +=
                    '<div>Pokemon No. ' + pvpUltraLeague[j]['pokemon'] + '-' + pvpUltraLeague[j]['form'] + ': ' + i8ln('CP too high') + '</div>'
            }
        }
...
function gymLabel(item) {
    ...
    if (raidSpawned && item.raid_end > Date.now() && !denyRaidLevels.includes(parseInt(item['raid_level']))) {
        ...
        if (raidStarted) {
            ...
            if (item['raid_level'] >= 6 && item['raid_evolution'] >= 1) { // Megaraid Evolution handling
                evolutionStr = '_' + item['raid_evolution']
    ...
    if (Store.get('badgeMode')) { // If Badgemode is active
        ...
        str =
            ...
            '</div>' +
            '</center></div>'
    } else if (teamId === 0 || (((lastScanned / 1000) > ((Date.now() / 1000) - 14400)) || noOutdatedGyms)) { // If normal mode
        if (!noWhatsappLink || !noRaidCounterGuide) {
            ...
            if ((!noWhatsappLink && (raidSpawned && item.raid_end > Date.now())) && (item.raid_pokemon_id > 1 && item.raid_pokemon_id < numberOfPokemon)) {
                if (!noWhatsappRaidMoves) {
                    bottomLine += '<a href="whatsapp://send?text=' + '📌%20%2A' + i8ln('Gym') + ':%2A%20%0A' + encodeURIComponent(item.name) + '%0A%0A⭐%20%2A' + i8ln('Raid') + ':%2A%20%0A' + i8ln('Level') + '%20' + item.raid_level + '%20' + item.raid_pokemon_name + '%0A%0A👊%20%2A' + i8ln('Moveset') + ':%2A%20%0A' + pMove1 + '/' + pMove2 + '%0A%0A🔛%20%2A' + i8ln('Raidtime') + ':%2A%20%0A' + i8ln('Start') + ':%20' + raidStartStr + '%0A' + i8ln('End') + ':%20' + raidEndStr + '%0A%0A🗺%20%2A' + i8ln('Route') + ':%2A%0Ahttps://maps.google.com/?q=' + item.latitude + ',' + item.longitude + '" data-action="share/whatsapp/share">' + i8ln('Whatsapp Link') + '</a>'
                } else {
                    bottomLine += '<a href="whatsapp://send?text=' + '📌%20%2A' + i8ln('Gym') + ':%2A%20%0A' + encodeURIComponent(item.name) + '%0A%0A⭐%20%2A' + i8ln('Raid') + ':%2A%20%0A' + i8ln('Level') + '%20' + item.raid_level + '%20' + item.raid_pokemon_name + '%0A%0A🔛%20%2A' + i8ln('Raidtime') + ':%2A%20%0A' + i8ln('Start') + ':%20' + raidStartStr + '%0A' + i8ln('End') + ':%20' + raidEndStr + '%0A%0A🗺%20%2A' + i8ln('Route') + ':%2A%0Ahttps://maps.google.com/?q=' + item.latitude + ',' + item.longitude + '" data-action="share/whatsapp/share">' + i8ln('Whatsapp Link') + '</a>'
                }
            } else if ((!noWhatsappLink) && (raidSpawned && item.raid_end > Date.now())) {
                bottomLine += '<a href="whatsapp://send?text=' + '📌%20%2A' + i8ln('Gym') + ':%2A%20%0A' + encodeURIComponent(item.name) + '%0A%0A⭐%20%2A' + i8ln('Raid') + ':%2A%20%0A' + i8ln('Level') + '%20' + item.raid_level + '%20Ei%20🥚%0A%0A🔛%20%2A' + i8ln('Raidtime') + ':%2A%20%0A' + i8ln('Start') + ':%20' + raidStartStr + '%0A' + i8ln('End') + ':%20' + raidEndStr + '%0A%0A🗺%20%2A' + i8ln('Route') + ':%2A%0Ahttps://maps.google.com/?q=' + item.latitude + ',' + item.longitude + '" data-action="share/whatsapp/share">' + i8ln('Whatsapp Link') + '</a>'
            }
            if (!noRaidCounterGuide && ((raidSpawned && item.raid_end > Date.now()) && (item.raid_pokemon_id > 1 && item.raid_pokemon_id < numberOfPokemon))) {
                if (!noWhatsappLink) {
                    bottomLine += hyphen
                }
                bottomLine += raidCounterGuideStr
            }
            if (((!noWhatsappLink) || (!noRaidCounterGuide && (item.raid_pokemon_id > 1 && item.raid_pokemon_id < numberOfPokemon))) && (raidSpawned && item.raid_end > Date.now())) {
                bottomLine += hyphen
            }
        }
        str =
    ...
    if (badgeMode) {
        ...
        if (item['raid_pokemon_id'] != null && item.raid_end > Date.now() && !denyRaidLevels.includes(parseInt(item['raid_level']))) {
            ...
            html = '<div style="position:relative;">' +
                '<img src="static/forts/' + Store.get('gymMarkerStyle') + '/' + teamStr + '.png" style="width:' + dynamicGymSize + 'px;height:auto;"/>' +
                exIcon +
                '<img src="static/raids/egg_' + item['raid_level'] + '.png" style="width:' + dynamicEggSize + 'px;height:auto;position:absolute;top:' + dynamicEggPosTop + 'px;right:' + dynamicEggPosRight + 'px;"/>' +
                exclusiveIcon +
                battleIcon +
                '</div>'
...
function setupGymMarker(item) {
    ...
    if (raidLevel >= Store.get('remember_raid_notify') && item.raid_end > Date.now() && Store.get('remember_raid_notify') !== 0) {
        ...
        if (raidStarted) {
            ...
            icon = 'static/raids/egg_hatched_' + item['raid_level'] + '.png'
        } else {
            icon = 'static/raids/egg_' + item['raid_level'] + '.png'
            checkAndCreateSound()
        }
...
function clearStaleMarkers() {
    $.each(mapData.pokemons, function (key, value) {
        ...
        if (!noPokePVPStats) {
            pvpGreatLeague = JSON.parse(mapData.pokemons[key]['pvp_gl'])
            pvpUltraLeague = JSON.parse(mapData.pokemons[key]['pvp_ul'])
        }
    ...
    $.getJSON('static/dist/data/mapstyle.min.json').done(function (data) {
        ...
        if (!forcedTileServer) {
            $.each(data, function (key, value) {
                ...
                var googleMaps = false // eslint-disable-line no-unused-vars
                if (gmapsKey !== '') {
                    googleMaps = true
                    if (googleStyle) {
                        styleList.push({
                ...
                var mapBox = false // eslint-disable-line no-unused-vars
                if (mBoxKey !== '') {
                    mapBox = true
                    if (mapBoxStyle) {
                        styleList.push({
